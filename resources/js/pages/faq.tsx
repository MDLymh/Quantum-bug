    ...
    <AppLayout>
      <div className="w-full max-w-4xl mx-auto py-10 px-4">
          ...
          {faqData.map((item, index) => (
            ...
            >
              <AccordionTrigger 
                ...
                className="text-left text-lg md:text-xl font-bold text-gray-200 hover:text-white hover:no-underline py-6 [&>svg]:hidden group"
              >
                {/* 1. Question Text on the left
                  flex-grow pushes the icon to the far right.
                */}
                <span className="flex-grow">{item.question}</span>
                
                {/* 2. CUSTOM ANIMATING "+" ICON (This is the key part!)
                  We use flex to align it, set a duration, and add transition-transform.
                  The key to rotation is "group-data-[state=open]:rotate-45", 
                  which makes it a "x" when open. 
                */}
                <div className="flex items-center justify-center p-2 rounded-full h-10 w-10 bg-qb-dark/50 border border-white/10">
                  <Plus 
                    className={cn(
                      "h-6 w-6 text-gray-400 shrink-0 transition-all duration-500", // Smooth 500ms spinning animation
                      "group-data-[state=open]:rotate-45 group-data-[state=open]:text-qb-cyan" // Spin to "x" and change to cyan color
                    )} 
                  />
                </div>
              </AccordionTrigger>
              
              <AccordionContent className="text-gray-400 text-base md:text-lg pb-8 leading-relaxed font-medium">
                {item.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
        ...
        <div className="mt-20 pt-10 border-t border-white/5 text-center">
            ...
            <p className="text-gray-500 text-sm mb-6 italic">If you can't find your answer here, feel free to contact our technical support team.</p>
            <a 
              href="/support" 
              className="inline-block bg-white text-qb-gray px-12 py-3 rounded-full font-black uppercase tracking-widest text-sm hover:bg-qb-cyan transition-all duration-300 shadow-md hover:shadow-[0_0_15px_rgba(47,244,238,0.3)] group"
            >
